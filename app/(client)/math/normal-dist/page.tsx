...
export default function NormalDist() {
    ...
    const { leftArea, rightArea } = useMemo(() => {
        ...
        xs.forEach((val) => {
            const point: Point = { x: val, pdf: normalPdf(val, 0, 1) };
            if (val <= x) leftArea.push(point);
            else rightArea.push(point);
        });
    ...
    const rowZ = Array.from({ length: 61 }, (_, i) =>
        (-3 + i * 0.1).toFixed(1)
    );
